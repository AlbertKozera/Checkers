...
namespace Warcaby.Service.Context
{
    public class GameService
    {
        ...
        public void Computer(string color)
        {
            ...
            {
                moveAndPoints = ai.MinMax(gameBoardCopy, color, true, 3); // MinMax start
            }
                
            if(readFirstLine == "Unchecked")
            {
                moveAndPoints = ai.MinMaxWithoutThreads(gameBoardCopy, color, true, 3); // MinMax_WithoutThreads start
            }
